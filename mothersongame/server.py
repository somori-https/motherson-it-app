...
def dashboard():
    ...
    # Using COALESCE ensures zero scores display properly instead of returning NULL
    ...
    user_best_row = conn.execute('''
        SELECT COALESCE(SUM(score), 0) as best_score FROM scores WHERE username = ?
    ''', (session['user'],)).fetchone()
    user_best = user_best_row['best_score'] if user_best_row else 0
    ...
    <div class="space-y-8">
        <div class="bg-zinc-950/90 backdrop-blur-md p-6 rounded-xl border border-zinc-800 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div class="bg-black p-4 rounded-xl border border-zinc-800 text-center min-w-[140px] w-full md:w-auto">
                ...
                <div class="text-2xl font-black text-white mt-0.5">{{ user_best }} <span class="text-xs font-normal text-zinc-400">PTS</span></div>
            </div>
        </div>
